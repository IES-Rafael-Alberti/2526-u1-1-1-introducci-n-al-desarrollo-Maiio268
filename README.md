...
Cuando el usuario escribe, el teclado (dispositivo de entrada) detecta una pulsación y envía señales al ordenador, estas señales se convierten en datos que el software puede usar, y estos datos se recogen y se almacenan temporalmente en la memoria RAM, después la CPU lee y ejecuta las instrucciones almacenadas en la RAM, procesa esos datos de entrada y produce la salida a través de la pantalla (dispositivo de salida). En C, Java y Python, el software gestiona la memoria especificando dónde se almacenan las variables y los datos durante la ejecución, por ejemplo, en C, se usan punteros para acceder a zonas específicas de memoria RAM, y las variables locales se gestionan en la pila del programa, en Java, las variables locales están en la pila y los objetos se almacenan en el montón (heap), y el procesador ejecuta el bytecode mediante la JVM, que interpreta o compila el código en tiempo real, en Python, la gestión de memoria es automática, ya que declaras variables dimplemente escribiendo el nombre, el intérprete juega su papel y finalmente el procesador ejecuta el código. Es decir, los 3 lenguajes funcionan de diferente manera.
...
- **Editor de texto o IDE (¿dónde escribiste el código?):** He utilizado el IDE Visual Studio Code, he usado este IDE para abrir el repositorio y he creado todas las carpetas y archivos necesarios, también he escrito los 3 programas con los 3 diferentes lenguajes de programación escribiendo en los nombres de los archivos la extensión correspondiente de cada lenguaje.
